...
Public Sub Render As MiniHtml
	...
	MH.Script.up(body1).src("https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.min.js") _
	.integrity("sha384-G/EV+4j2dNv+tEPo3++6LCgdCROaejBqfUeNjuKAiuXbjrxilcCdDz6ZAVfHWe1Y").crossorigin("anonymous").wrapAttributes
	MH.Script.up(body1).src("https://cdn.jsdelivr.net/npm/htmx.org@2.0.8/dist/htmx.min.js") _
	.integrity("sha384-/TgkGk7p307TH7EXJDuUlgG3Ce1UVolAOFopFekQkkXihi5u/6OCvVKyz1W+idaz").crossorigin("anonymous").wrapAttributes
	MH.Script.up(body1).src("$SERVER_URL$/assets/js/app.js")
	Return page1
End Sub

Private Sub PageHeader As MiniHtml
	Dim head1 As MiniHtml = MH.ResponsiveHeader
	MH.Meta.up(head1).attr("name", "description").attr("content", "Generated using MiniHTML B4X library")
	...
	MH.Link.up(head1).href("https://cdn.jsdelivr.net/npm/bootstrap-icons@1.13.1/font/bootstrap-icons.min.css")
	MH.Link.up(head1).href("$SERVER_URL$/assets/css/main.css?v=$VERSION$")
	Return head1
End Sub
...
Private Sub PageBody As MiniHtml
	...
	MH.NavbarToggler.up(div1).wrapAttributes.comment("toggler")
