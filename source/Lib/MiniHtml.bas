...
Public Sub Initialize (Name As String)
	...
	mBuilder.Initialize
	mFlat = False
	mLineFeed = True 'Not(mFlat)
	mIndentation = True 'Not(mFlat)
	mDocType = "html"
	mName = Name
	Select mName.ToLowerCase
		...
		Case "meta", "input", "link"
			mMode = mMeta
		Case "title", "h1", "h2", "h3", "h4", "h5", "h6", "p", "script", "label", "button", "div", "span", "li", "a", "i", "b", "u", "option", "bold", "italic", "underline", "strong", "em", "del", "th", "td", "small", "textarea"
			mMode = mUniline
		Case "img", "br", "path"
			mMode = mMeta
		Case "text", ""
...
Public Sub buildImpl (indent As Int, AlignAttribute2 As Boolean) As String
	...
	If mMode <> "" And mName <> mNoTag Then
		SB.Append("<" & mName)
	End If
...
Public Sub multilineIf (condition As Boolean) As MiniHtml
	If condition Then mMode = mMultiline
	Return Me
End Sub
...
''Add a comment as child
'Public Sub addComment (value As String, newline As Boolean = False) As MiniHtml
...
Public Sub comment (value As String, position As String = "above") As MiniHtml
	...
	If Initialized(ctp) Then
		...
		If pos > -1 Then
			Dim nt As MiniHtml = Create(mNoTag).text($"<!--${value.Replace("--", "")}-->"$)
...
'Public Sub name (value As String) As MiniHtml
'	mAttributes.Put("name", value)
'	Return Me
'End Sub

'Public Sub content (value As String) As MiniHtml
'	mAttributes.Put("content", value)
'	Return Me
'End Sub

'Add type attribute
'Public Sub atype (value As String) As MiniHtml
...
'	mChildren.Add(Create(mNoTag).text(value).Indentation = True)
'	Return Me
'End Sub
